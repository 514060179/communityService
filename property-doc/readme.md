...
@NewlandParamsDoc(params = {
        @NewlandParamsDoc(name = "username", length = 30, remark = "用户名，物业系统分配"),
        @NewlandParamsDoc(name = "passwd", length = 30, remark = "密码，物业系统分配"),
})

@NewlandResponseDoc(
        params = {
                @NewlandParamsDoc(name = "code", type = "int", length = 11, defaultValue = "0", remark = "返回编号，0 成功 其他失败"),
                @NewlandParamsDoc(name = "msg", type = "String", length = 250, defaultValue = "成功", remark = "描述"),
                @NewlandParamsDoc(name = "data", type = "Object", remark = "有效数据"),
                @NewlandParamsDoc(parentNodeName = "data",name = "userId", type = "String", remark = "用户ID"),
                @NewlandParamsDoc(parentNodeName = "data",name = "token", type = "String", remark = "临时票据"),
        }
)
...
server {
	...
	location / {
	   proxy_pass http://dev.api.property.newland.com:8008/doc-ui.html;
	}
    location /js {
	   proxy_pass http://dev.api.property.newland.com:8008;
	}
    location /css {
	   proxy_pass http://dev.api.property.newland.com:8008;
	}
	location /doc
	{
	  ...
	  proxy_pass   http://dev.api.property.newland.com:8088;
	}
}
```
