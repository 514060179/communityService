package com.newland.property.boot.importData.adapt;
...
import com.newland.property.boot.importData.DefaultImportDataAdapt;
import com.newland.property.boot.importData.IImportDataCleaningAdapt;
...
//            Assert.hasValue(os[9], (osIndex + 1) + "行业主手机号不能为空");
            ...
            importOwnerInfoDto.setSex(os[1].toString().trim().equals("男") ? "0" : "1");
            ...
            if (os.length >= 7 && os[6] != null) {
                importOwnerInfoDto.setAddress(os[6].toString().trim());
            } else {
                importOwnerInfoDto.setAddress("");
            }

            if (os.length >= 8 && os[7] != null) {
                importOwnerInfoDto.setRemark(os[7].toString().trim());
            } else {
                importOwnerInfoDto.setRemark("");
            }

            if (os.length >= 9 && os[8] != null) {
                importOwnerInfoDto.setOwnerAreaCode(os[8].toString().trim());
            } else {
                importOwnerInfoDto.setOwnerAreaCode("");
            }

            if (os.length >= 10 && os[9] != null) {
                importOwnerInfoDto.setOwnerLink(os[9].toString().trim());
            } else {
                importOwnerInfoDto.setOwnerLink("");
            }

            importOwnerInfoDto.setState("2000");
            importOwnerInfoDto.setbId(GenerateCodeFactory.getBId());
...
//            importOwnerInfoDto.setRoomState(os[11].toString().trim());
    ...
    private void validateOwnerInfo(List<ImportOwnerInfoDto> ownerInfoDtos) {
        ...
        for (int roomIndex = 0; roomIndex < ownerInfoDtos.size(); roomIndex++) {
            ...
            if(!importOwnerInfoDto.getOwnerTypeCd().equals(OwnerDto.OWNER_TYPE_CD_OWNER)) {
                if (StringUtil.isEmpty(importOwnerInfoDto.getOwnerAreaCode())) {
                    throw new IllegalArgumentException((roomIndex + 2) + "行业主手机区号不能为空");
                }

                if (StringUtil.isEmpty(importOwnerInfoDto.getOwnerLink())) {
                    throw new IllegalArgumentException((roomIndex + 2) + "行业主手机号码不能为空");
                }
            }
