...
import com.newland.property.core.component.AbstractComponentSMO;
...
import com.newland.property.utils.cache.PrivilegeCache;
...
public class DefaultAbstractComponentSMO extends AbstractComponentSMO {
    ...
    protected ComponentValidateResult validateStoreStaffCommunityRelationship(IPageData pd, RestTemplate restTemplate) {
        ...
        if (!StringUtil.isEmpty(users.get(0).getLevelCd()) && !"02".equals(users.get(0).getLevelCd())) { //02表示普通用户
            // 校验 员工和商户是否有关系
