package com.newland.property.boot.aop;
...
public class PageProcessAspect {
    ...
    @Pointcut("execution(public * com.newland.property..*.*Controller.*(..)) ")
    ...
    public void deBefore(JoinPoint joinPoint) throws Throwable {
        ...
        headers.put("_newland_token",this.getToken(request));
        pd = PageData.newInstance().builder(userId, userName, "", reqData, componentCode, componentMethod, url, sessionId, appId, headers);
        pd.setMethod("GET".equals(request.getMethod()) ? HttpMethod.GET : HttpMethod.POST);

        logger.debug("切面 获取到的pd=" + JSONObject.toJSONString(pd));
        request.setAttribute(CommonConstant.CONTEXT_PAGE_DATA, pd);
        //调用链
        //NewlandPropertyTraceFactory.createTrace(componentCode + "/" + componentMethod, headers);
    }
