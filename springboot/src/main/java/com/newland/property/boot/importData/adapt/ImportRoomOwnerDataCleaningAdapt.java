...
import com.newland.property.dto.importData.ImportOwnerRoomDto;
    ...
    private void getOwnerRooms(Workbook workbook, List<ImportOwnerRoomDto> ownerRoomDtos, ComponentValidateResult result) throws ParseException {
        ...
        for (int osIndex = 0; osIndex < oList.size(); osIndex++) {
            ...
            if (os.length > 12 && !StringUtil.isNullOrNone(os[12])) {
                ...
                Assert.hasValue(os[14], (osIndex + 1) + "行手机区号不能为空");
            ...
            if (os.length > 12 && !StringUtil.isNullOrNone(os[12])) {
                ...
                importOwnerRoomDto.setAreaCode(os[14].toString().trim());
                importOwnerRoomDto.setTel(os[15].toString().trim());
                String idCard = StringUtil.isNullOrNone(os[16]) ? "" : os[16].toString().trim();
...
//            }
            ...
            //烦请专业测试团队勿喷
            if (StringUtil.isEmpty(importOwnerRoomDto.getAreaCode())) {
                throw new IllegalArgumentException((roomIndex + 2) + "行手机号区号不能为空");
            }
