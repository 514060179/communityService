package com.newland.property.job.adapt.purchase.purchaseApply;
...
public class MachinePurchaseApplyAdapt extends DatabusAdaptImpl {
    ...
    private void sendMessage(JSONObject paramIn) {
        ...
        if (ListUtil.isNull(smallWechatAttrDtos)) {
        ...
        if (staffAppAuthDtos.size() > 0) {
            ...
            if (!StringUtil.isEmpty(resOrderType) && "10000".equals(resOrderType)) {  //采购入库
            ...
            if (!StringUtil.isEmpty(resOrderType) && "10000".equals(resOrderType)) {
            ...
            if (!StringUtil.isEmpty(noticeState) && "1001".equals(noticeState)) { //审核中
                data.setKeyword5(new Content("审核中"));
            } else if (!StringUtil.isEmpty(noticeState) && "1004".equals(noticeState)) {
                data.setKeyword5(new Content("未通过" + "\r\n" + paramIn.getString("auditMessage")));
            } else if (!StringUtil.isEmpty(noticeState) && "1002".equals(noticeState)) {
                data.setKeyword5(new Content("已审核"));
            } else {
                data.setKeyword5(new Content(paramIn.getString("stateName")));
            }
            if (!StringUtil.isEmpty(resOrderType) && "10000".equals(resOrderType) && !StringUtil.isEmpty(noticeState) && "1004".equals(noticeState)) {  //采购审核不通过
                data.setRemark(new Content("您的采购申请审核未通过，请结束流程！"));
            } else if (!StringUtil.isEmpty(resOrderType) && "20000".equals(resOrderType) && !StringUtil.isEmpty(noticeState) && "1004".equals(noticeState)) {  //领用审核不通过
                data.setRemark(new Content("您的领用申请审核未通过，请结束流程！"));
            } else if (!StringUtil.isEmpty(resOrderType) && "10000".equals(resOrderType) && !StringUtil.isEmpty(noticeState) && "1002".equals(noticeState)) {
                data.setRemark(new Content("您的采购申请已审核，请结束流程！"));
            } else if (!StringUtil.isEmpty(resOrderType) && "20000".equals(resOrderType) && !StringUtil.isEmpty(noticeState) && "1002".equals(noticeState)) {
