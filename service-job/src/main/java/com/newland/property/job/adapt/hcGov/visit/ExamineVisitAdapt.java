package com.newland.property.job.adapt.hcGov.visit;
...
public class ExamineVisitAdapt extends DatabusAdaptImpl {
        ...
        for (int bVisitIndex = 0; bVisitIndex < businessVisits.size(); bVisitIndex++) {
            ...
            if (!StringUtil.isEmpty(businessVisit.getString("carNum"))) {
                ...
                if (visitSettingDtos == null || visitSettingDtos.size() != 1 || StringUtil.isEmpty(visitSettingDtos.get(0).getCarNumWay())
                        || !"Y".equals(visitSettingDtos.get(0).getCarNumWay())) { //访客配置信息不存在，或者访客配置信息里面的车辆不同步，就不推送信息
                    return;
                }
                if (!StringUtil.isEmpty(visitSettingDtos.get(0).getAuditWay()) && "N".equals(visitSettingDtos.get(0).getAuditWay())) { //物业审核 Y 是 N 否
                    if (!StringUtil.isEmpty(businessVisit.getString("carState")) &&
                            ("0".equals(businessVisit.getString("carState")) || "3".equals(businessVisit.getString("carState")))) { //不需要流程人审核，车辆状态为待审核或审核中状态时，给员工推送消息
                        publishMsg(business, businessVisit); //给有预约车审核权限的员工推送消息
                    }
                } else { //需要流程人审核
                    if (!StringUtil.isEmpty(businessVisit.getString("state")) && "1".equals(businessVisit.getString("state"))) { //访客记录为审核通过状态
                        if (!StringUtil.isEmpty(businessVisit.getString("carState")) &&
                                ("0".equals(businessVisit.getString("carState")) || "3".equals(businessVisit.getString("carState")))) { //访客记录通过，车辆状态为待审核或审核中状态时，给员工推送消息
                            publishMsg(business, businessVisit); //给有预约车审核权限的员工推送消息
                        }
                    }
                }
                if (!StringUtil.isEmpty(businessVisit.getString("carState")) &&
                        ("1".equals(businessVisit.getString("carState")) || "2".equals(businessVisit.getString("carState")))) { //车辆必须为审核通过或审核拒绝状态
    ...
    private void sendMessage(Business business, JSONObject businessVisit) {
        ...
        if (ownerAppUserDtos.size() > 0) {
            ...
            if (!StringUtil.isEmpty(visitDtos.get(0).getCarState()) && "1".equals(visitDtos.get(0).getCarState())) { //车辆审核状态 0表示未审核；1表示审核通过；2表示审核拒绝；3审核中
