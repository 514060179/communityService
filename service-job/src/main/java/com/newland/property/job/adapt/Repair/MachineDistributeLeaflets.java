package com.newland.property.job.adapt.Repair;
...
import com.newland.property.dto.printerRule.PrinterRuleDto;
import com.newland.property.dto.printerRule.PrinterRuleMachineDto;
import com.newland.property.dto.printerRule.PrinterRuleRepairDto;
...
public class MachineDistributeLeaflets extends DatabusAdaptImpl {
    ...
    private void doDealOwnerRepair(List<Business> businesses, JSONObject businessRepairUser) {
        ...
        if ("1100".equals(repairState) && "10006".equals(businessRepairUser.getString("state"))) { //派单
            JSONObject paramIn = new JSONObject();
            for (Business business : businesses) {
                String businessTypeCd = business.getBusinessTypeCd();
                if (!StringUtil.isEmpty(businessTypeCd) && "130200030001".equals(businessTypeCd)) {
                    JSONObject data = business.getData();
                    if (!StringUtil.isEmpty(data.getString("state")) && "10001".equals(data.getString("state"))) {
                        paramIn.put("staffId", data.getString("staffId"));
                        paramIn.put("staffName", data.getString("staffName"));
                    } else if ("10006".equals(data.getString("state"))) {
        ...
        } else if ("1100".equals(repairState) && !"10006".equals(businessRepairUser.getString("state"))) {
            JSONObject paramIn = new JSONObject();
            ...
            //抢单成功给业主推送信息
            sendOwnerMsg(paramIn, communityDtos.get(0),msgNotify);
        } else if ("1300".equals(repairState)) {   //转单
            JSONObject paramIn = new JSONObject();
            paramIn.put("repairName", repairName);
            paramIn.put("repairObjName", repairObjName);
            paramIn.put("tel", tel);
