...
//                publishMsg(business, businessVisit); //访客登记给有预约车审核权限的员工推送消息
    ...
    private void publishMsg(Business business, JSONObject businessVisit) {
        ...
        if (!StringUtil.isEmpty(visitDtos.get(0).getState()) && "1".equals(visitDtos.get(0).getState())) { //访客审核状态 0表示未审核；1表示审核通过；2表示审核拒绝；3表示审核中
            return;
        }
        //查询访客配置信息
        ...
        if (visitSettingDtos == null || visitSettingDtos.size() != 1 || StringUtil.isEmpty(visitSettingDtos.get(0).getCarNumWay())
                || !"Y".equals(visitSettingDtos.get(0).getCarNumWay()) || StringUtil.isEmpty(visitSettingDtos.get(0).getAuditWay())
                || "Y".equals(visitSettingDtos.get(0).getAuditWay())) { //访客配置信息不存在，或者访客配置信息里面的车辆不同步，或者访客配置访客记录需要流程人审核，就不推送信息
            return;
        }
        //车辆待审核状态，给有审核权限的员工推送消息
        if (!StringUtil.isEmpty(visitDtos.get(0).getCarState()) &&
                ("0".equals(visitDtos.get(0).getCarState()) || "3".equals(visitDtos.get(0).getCarState()))) { //车辆审核状态 0表示未审核；1表示审核通过；2表示审核拒绝；3审核中
        ...
        } else if (!StringUtil.isEmpty(visitDtos.get(0).getCarState()) && ("1".equals(visitDtos.get(0).getCarState()) || "2".equals(visitDtos.get(0).getCarState()))) { //车辆不需要审核，此时给业主发信息
//            sendMessage(visitDtos); //给业主推送消息
    ...
    private void sendMessage(List<VisitDto> visitDtos) {
        ...
        if (ownerAppUserDtos.size() > 0) {
            ...
            if (!StringUtil.isEmpty(visitDtos.get(0).getCarState()) && "1".equals(visitDtos.get(0).getCarState())) { //审核通过
