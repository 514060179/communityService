...
@NewlandPropertyCmd(serviceCode = "ownerRepair.listOwnerRepairs")
public class ListOwnerRepairsCmd extends Cmd {
    ...
    public void doCmd(CmdEvent event, ICmdDataFlowContext context, JSONObject reqJson) throws CmdException, ParseException {
        ...
        if (!StringUtil.isEmpty(ownerRepairDto.getReqSource()) && "mobile".equals(ownerRepairDto.getReqSource())) {
            ownerRepairDto.setState(RepairDto.STATE_WAIT);
        }
        //todo pc电话报修模块 只返回PC员工登记和手机端员工登记的数据
        if (!StringUtil.isEmpty(ownerRepairDto.getReqSource()) && "pc_mobile".equals(ownerRepairDto.getReqSource())) {
