...
import com.newland.property.community.bmo.room.IQueryRoomStatisticsBMO;
...
@NewlandPropertyCmdDoc(title = "查询房屋",
        description = "查询房屋信息",
        httpMethod = "get",
...
@NewlandPropertyResponseDoc(
        params = {
                ...
                @NewlandPropertyParamDoc(name = "rooms", type = "Object", remark = "有效数据"),
                @NewlandPropertyParamDoc(parentNodeName = "rooms", name = "roomName", type = "String", remark = "房屋名称"),
                @NewlandPropertyParamDoc(parentNodeName = "rooms", name = "roomId", type = "String", remark = "房屋编号"),
        }
)
...
@NewlandPropertyCmd(serviceCode = "room.queryRooms")
    ...
    private void refreshRoomOwners(String userId, String communityId, List<RoomDto> roomDtos) {
        ...
        for (RoomDto roomDto : roomDtos) {
            for (OwnerDto tmpOwnerDto : ownerDtos) {
                ...
                if (mark.size() == 0 && idCard != null && !"".equals(idCard) && idCard.length() > 15) {
                    idCard = idCard.substring(0, 6) + "**********" + idCard.substring(16);
                }
                //对业主手机号隐藏处理
                String link = tmpOwnerDto.getLink();
                roomDto.setOwnerTel(link); //程序用 主要用以查询 报修等统计信息
                if (mark.size() == 0 && link != null && !"".equals(link) && link.length() > 10) {
