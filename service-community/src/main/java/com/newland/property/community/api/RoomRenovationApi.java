...
import com.newland.property.community.bmo.roomRenovation.IDeleteRoomRenovationBMO;
...
import com.newland.property.community.bmo.roomRenovationDetail.IDeleteRoomRenovationDetailBMO;
import com.newland.property.community.bmo.roomRenovationDetail.IGetRoomRenovationDetailBMO;
...
import com.newland.property.community.bmo.roomRenovationRecord.ISaveRoomRenovationRecordBMO;
...
public class RoomRenovationApi {
    ...
    public ResponseEntity<String> saveRoomRenovation(@RequestBody JSONObject reqJson,
        ...
        if (roomRenovationDtos.size() > 0) {
            for (RoomRenovationDto renovationDto : roomRenovationDtos) {
                if ("1000".equals(renovationDto.getState()) || "3000".equals(renovationDto.getState()) || "4000".equals(renovationDto.getState())) {
    ...
    public ResponseEntity<String> updateRoomRenovation(@RequestBody JSONObject reqJson) {
        ...
        if (!StringUtil.isEmpty(roomRenovationPo.getIsPostpone()) && "N".equals(roomRenovationPo.getIsPostpone())) { //是否延期 Y延期 N正常
            roomRenovationPo.setPostponeTime(null); //延期时间
        }
        if (!StringUtil.isEmpty(roomRenovationPo.getIsViolation()) && "N".equals(roomRenovationPo.getIsViolation())) { //是否违规 Y违规 N正常
            roomRenovationPo.setViolationDesc(null); //违规说明
        }
        //如果状态为装修中、待验收，则房屋状态改为装修中；如果状态为验收成功，则房屋状态改为已装修；如果为待审核、审核失败、验收失败，则房屋状态改为已交房
        if ("3000".equals(roomRenovationPo.getState()) || "4000".equals(roomRenovationPo.getState())) {
        ...
        } else if ("1000".equals(roomRenovationPo.getState()) || "2000".equals(roomRenovationPo.getState())
                || "5000".equals(roomRenovationPo.getState())) {
            RoomDto roomDto = new RoomDto();
            roomDto.setRoomId(roomRenovationPo.getRoomId());
            //房屋状态变为已交房
            ...
            updateRoomRenovationBMOImpl.update(roomRenovationPo);
            return updateRoomRenovationBMOImpl.updateRoom(roomDto);
        } else if ("6000".equals(roomRenovationPo.getState())) {
            RoomDto roomDto = new RoomDto();
            roomDto.setRoomId(roomRenovationPo.getRoomId());
            //房屋状态变为已装修
...
//        }
    ...
    public ResponseEntity<String> updateRoomToExamine(@RequestBody JSONObject reqJson) {
        RoomRenovationPo roomRenovationPo = BeanConvertUtil.covertBean(reqJson, RoomRenovationPo.class);
        //审核通过房屋状态变为装修中
        if ("3000".equals(roomRenovationPo.getState())) {
            RoomDto roomDto = new RoomDto();
            roomDto.setRoomId(roomRenovationPo.getRoomId());
            //房屋状态变为装修中
            roomDto.setState("2009");
            ...
            updateRoomRenovationBMOImpl.update(roomRenovationPo);
            return updateRoomRenovationBMOImpl.updateRoom(roomDto);
        } else if ("2000".equals(roomRenovationPo.getState())) {
            RoomDto roomDto = new RoomDto();
            roomDto.setRoomId(roomRenovationPo.getRoomId());
            //房屋状态变为已交房
            roomDto.setState("2003");
    ...
    public ResponseEntity<String> saveRoomRenovationDetail(@RequestHeader(value = "user-id") String userId,
        ...
        updateRoomRenovationBMOImpl.update(roomRenovationPo);
        //验收成功后房屋状态变为已装修
        if ("5000".equals(roomRenovationDetailPo.getState())) {
            RoomDto roomDto = new RoomDto();
            ...
            return updateRoomRenovationBMOImpl.updateRoom(roomDto);
        } else if ("6000".equals(roomRenovationDetailPo.getState())) { //验收失败装修状态变为装修中
