...
import com.newland.property.intf.store.IUserStorehouseV1InnerServiceSMO;
...
@NewlandPropertyCmd(serviceCode = "ownerRepair.repairFinish")
...
//        Assert.hasKeyAndValue(reqJson, "amount", "未包含金额");
    ...
    public void doCmd(CmdEvent event, ICmdDataFlowContext context, JSONObject reqJson) throws CmdException, ParseException {
        ...
        if (!ListUtil.isNull(json) && ("1001".equals(maintenanceType) || "1003".equals(maintenanceType))) {
            ...
            for (int i = 0; i < objects.length; i++) {
                ...
                if (!StringUtil.isEmpty(useNumber) && !"0".equals(useNumber)
                        && (!StringUtil.isEmpty(isCustom) && "false".equals(isCustom))) {
                    String nowStock = "0";
                    //（从我的物料中获取商品库存）
                ...
                if ("1001".equals(maintenanceType) && (!StringUtil.isEmpty(isCustom) && "false".equals(isCustom))) {
            ...
            for (int i = 0; i < objects.length; i++) {
                ...
                if (!StringUtil.isEmpty(isCustom) && "true".equals(isCustom)) {
                    resId = "666666";
                ...
                } else if ("1003".equals(maintenanceType)) {  //需要用料
                    flag = resourceStoreUseRecordV1InnerServiceSMOImpl.saveResourceStoreUseRecord(resourceStoreUseRecordPo);
                    if (flag < 1) {
                        throw new CmdException("添加失败");
                    }
        ...
        // TODO 完成保修后发推送-jpush
        // appPage: /pages/repair/myRepair?activeTab=2  // 已完成
        // appPage: /pages/repair/appraiseRepair?repairId=xxx   // 待评价

        JPushMessageDto messageDto = new JPushMessageDto();
        messageDto.setCommunityId(repairUserDtos.get(0).getCommunityId());
        messageDto.setUserId(repairUserDtos.get(0).getStaffId());
        messageDto.setPageUrl("/pages/repair/myRepair?activeTab=2");
        ...
        messageDto1.setCommunityId(repairUserDtos.get(0).getCommunityId());
        messageDto1.setUserId(repairUserDtos.get(0).getStaffId());
        messageDto1.setPageUrl("/pages/repair/appraiseRepair?repairId=" + repairUserDtos.get(0).getRepairId());
