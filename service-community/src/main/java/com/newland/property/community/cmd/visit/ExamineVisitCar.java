 ...
 */
@NewlandPropertyCmd(serviceCode = "visit.examineVisitCar")
public class ExamineVisitCar extends Cmd {
    ...
    public void doCmd(CmdEvent event, ICmdDataFlowContext context, JSONObject reqJson) throws CmdException, ParseException {
        ...
        if (reqJson.containsKey("carState") && "1".equals(reqJson.getString("carState"))) { //审核通过
...
//        String freeTime = CommunitySettingFactory.getValue(reqJson.getString("communityId"), CAR_FREE_TIME);
    ...
    public JSONObject getVisitCarOperate(JSONObject reqJson) {
        ...
        if (visitSettingDtos != null && visitSettingDtos.size() > 0) {
            ...
            if (!StringUtil.isEmpty(visitSettingDtos.get(0).getCarNumWay()) && "Y".equals(visitSettingDtos.get(0).getCarNumWay())) { //车辆是否同步 Y 是 N 否
