...
public class CommunitySpaceV1ServiceDaoImpl extends BaseServiceDao implements ICommunitySpaceV1ServiceDao {
    ...
    @Override
    public List<Map> getCommunitySpaceFileInfo(Map info) throws DAOException {
        logger.debug("查询 getCommunitySpaceFileInfo 入参 info : {}",info);

        List<Map> businessCommunitySpaceInfos = sqlSessionTemplate.selectList("communitySpaceV1ServiceDaoImpl.getCommunitySpaceFileInfo",info);

        return businessCommunitySpaceInfos;
    }

    @Override
    public int queryCommunitySpaceFilesCount(Map info) {
        logger.debug("查询 queryCommunitySpaceFilesCount 入参 info : {}",info);

        List<Map> businessCommunitySpaceInfos = sqlSessionTemplate.selectList("communitySpaceV1ServiceDaoImpl.queryCommunitySpaceFilesCount", info);
        if (businessCommunitySpaceInfos.size() < 1) {
            return 0;
        }
