...
//@NewlandPropertyCmd(serviceCode = "visit.saveVisit")
public class SaveVisitSdCmd extends Cmd {
    ...
    public void doCmd(CmdEvent event, ICmdDataFlowContext context, JSONObject reqJson) throws CmdException {
        ...
        if (reqJson.containsKey("carNum") && !StringUtil.isEmpty(reqJson.getString("carNum"))) {
            ...
            if (!StringUtils.isEmpty(isNeedReviewFlag) && "false".equals(isNeedReviewFlag)) { //不需要审核就随机自动分配车位
                //获取小区配置里配置的停车场id
            ...
            if (count >= number && number > 0) {
                if (!StringUtils.isEmpty(isNeedReviewFlag) && "false".equals(isNeedReviewFlag)) {
                    reqJson.put("state", "0");
