...
import com.newland.property.intf.acct.INotifyPaymentV1InnerServiceSMO;
...
public class NotifyPaymentV1InnerServiceSMOImpl extends BaseServiceSMO implements INotifyPaymentV1InnerServiceSMO {
    ...
    protected static final String DEFAULT_BOCPAY_PAYMENT_ADAPT = "bocpayFactoryAdapt";// 默认中银智慧付通用支付
...
//支付适配器IPayNotifyAdapt
            ...
            PaymentOrderDto paymentOrderDto = tPayNotifyAdapt.newlandPropertyNotifyPayment(notifyPaymentOrderDto);
    ...
    @Override
    public ResponseEntity<String> bocpayNotifyPayment(NotifyPaymentOrderDto notifyPaymentOrderDto) {
        String payNotifyAdapt = MappingCache.getValue(BocpayConstant.BOCPAY_DOMAIN,
                BocpayConstant.BOCPAY_PAYMENT_ADAPT);

        payNotifyAdapt = StringUtil.isEmpty(payNotifyAdapt) ? DEFAULT_BOCPAY_PAYMENT_ADAPT : payNotifyAdapt;
        ...
        String paramIn = CommonCache.getAndRemoveValue("bocpayPayment_" + paymentOrderDto.getOrderId());
