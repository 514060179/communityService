...
import com.newland.property.vo.api.ApiOwnerDataVo;
import com.newland.property.vo.api.ApiOwnerVo;
...
@NewlandPropertyCmdDoc(title = "查询业主",
...
@NewlandPropertyParamsDoc(params = {
        ...
        @NewlandPropertyParamDoc(name = "ownerId", length = 30, remark = "业主ID，家庭成员业主ID"),
        @NewlandPropertyParamDoc(name = "memberId", length = 30, remark = "业主ID，主键ID"),
})
...
@NewlandPropertyCmd(serviceCode = "owner.queryOwners")
...
//        }
        ...
        if (params.length > 2) {
            roomDto.setRoomNumLike(params[2]);
        }
        roomDto.setCommunityId(reqJson.getString("communityId"));
        List<RoomDto> roomDtos = roomInnerServiceSMOImpl.queryRooms(roomDto);
        Assert.listNotNull(roomDtos, "未查询到房屋下业主信息");
        if (roomDtos.size() == 1) {
            reqJson.put("roomId", roomDtos.get(0).getRoomId());
        } else {
            List<String> roomIds = new ArrayList<>();
            for (RoomDto roomDto1 : roomDtos) {
                roomIds.add(roomDto1.getRoomId());
            }
            reqJson.put("roomIds", roomIds.toArray(new String[roomIds.size()]));
        }
    }
