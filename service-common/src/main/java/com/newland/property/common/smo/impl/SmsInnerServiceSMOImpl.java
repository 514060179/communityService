...
public class SmsInnerServiceSMOImpl extends BaseServiceSMO implements ISmsInnerServiceSMO {
    ...
    public SmsDto send(@RequestBody SmsDto smsDto) {
        ...
        try {
            ...
            SendSmsFactory.sendSms(smsDto.getAreaCode(),smsDto.getTel(), smsDto.getCode());
    ...
    public SmsDto validateCode(@RequestBody SmsDto smsDto) {
        ...
        //校验是否有有效的验证码
        String smsCode = CommonCache.getValue(smsDto.getAreaCode() + smsDto.getTel() + SendSmsFactory.VALIDATE_CODE);
        ...
        if (smsDto.getCode().equals(smsCode)) {
            ...
            smsDto.setMsg("校验成功");
            //将验证码清除
            CommonCache.setValue(smsDto.getAreaCode() + smsDto.getTel() + SendSmsFactory.VALIDATE_CODE, "", 0);
            return smsDto;
        }
        smsDto.setSuccess(false);
