 ...
 * @author Moonny
 */
@Service("addMemberSendMessageAdapt")
public class AddMemberSendMessageAdapt implements ISendJPushMessageAdapt {

    private static final String DEFAULT_TITLE = "家庭成员";

    private static final String DEFAULT_CONTENT = "添加家庭成员成功";
    ...
    public void sendMessage(String communityId, String userId) {
        String title = MappingCache.getValue(JPushConstant.JPUSH_DOMAIN, JPushConstant.ADD_MEMBER_MESSAGE_TITLE);
        String content = MappingCache.getValue(JPushConstant.JPUSH_DOMAIN, JPushConstant.ADD_MEMBER_MESSAGE_CONTENT);
        String appUrl = MappingCache.getValue(JPushConstant.JPUSH_DOMAIN, JPushConstant.ADD_MEMBER_MESSAGE_APP_URL);
