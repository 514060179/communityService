package com.newland.property.fee.bmo.impl;
...
public class QueryOweFeeImpl implements IQueryOweFee {
    ...
    public ResponseEntity<String> querys(FeeDto feeDto) {
        ...
        int threadNum = PropertyThreadPoolFactory.NEWLANDPROPERTY_DEFAULT_THREAD_NUM;
    ...
    private List<RoomDto> doGetTmpRoomDto(List<RoomDto> roomDtos, FeeDto feeDto, int threadNum) {
        ...
        try {
            propertyThreadPoolFactory = PropertyThreadPoolFactory.getInstance().createThreadPool(threadNum);
            for (RoomDto roomDto1 : roomDtos) {
                propertyThreadPoolFactory.submit(() -> {
                    return getTmpRoomDtos(roomDto1, feeDto);
                });
            }
            return propertyThreadPoolFactory.get();
        } finally {
