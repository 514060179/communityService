...
import com.newland.property.dto.fee.*;
...
import com.newland.property.fee.smo.impl.FeeReceiptInnerServiceSMOImpl;
...
import com.newland.property.utils.constant.FeeFlagTypeConstant;
 ...
 * 这个类已经很复杂 ，最好不要加新逻辑
 */
@NewlandPropertyCmd(serviceCode = "fee.payFee")
...
//        }
    ...
    public void doCmd(CmdEvent event, ICmdDataFlowContext cmdDataFlowContext, JSONObject paramObj) throws CmdException {
        ...
        try {
            ...
            ownerCarDto.setCommunityId(payFeePo.getCommunityId());
            ownerCarDto.setCarId(payFeePo.getPayerObjId());
...
//                integralSum = integralSum.add(subtract);
    ...
    private void doDealRepairOrder(JSONObject paramObj) {
        ...
        if (feeAttrDtos != null && feeAttrDtos.size() > 0) {
            ...
            if ("Z".equals(repairChannel)) { //如果是业主自主报修，状态就变成待评价
    ...
    public void dealAccount(JSONObject paramObj) {
        ...
        for (int columnIndex = 0; columnIndex < jsonArray.size(); columnIndex++) {
            ...
            JSONObject param = jsonArray.getJSONObject(columnIndex);
            if (!StringUtil.isEmpty(param.getString("acctType")) && "2004".equals(param.getString("acctType"))) { //积分账户
                //获取抵扣比例
                BigDecimal deductionProportion = new BigDecimal(param.getString("deductionProportion"));
                //计算实收款所扣的积分
