...
import com.newland.property.dto.report.ReportDeposit;
...
import com.newland.property.report.bmo.reportFeeMonthStatistics.IGetReportFeeMonthStatisticsBMO;
            ...
            for (RepairUserDto repairUser : repairUsers) {
                ...
                if (repairUserDtoList != null && repairUserDtoList.size() > 0) {
                    ...
                    for (RepairUserDto repair : repairUserDtoList) {
                        //处理中状态
                        if ("10001".equals(repair.getState())) {
                            //获取数量
                            int amount = Integer.parseInt(repair.getAmount());
                            dealAmount = dealAmount + amount;
                        } else if ("10002".equals(repair.getState())) {  //结单状态
                            //获取数量
                            int amount = Integer.parseInt(repair.getAmount());
                            statementAmount = statementAmount + amount;
                        } else if ("10003".equals(repair.getState())) {  //退单状态
                            //获取数量
                            int amount = Integer.parseInt(repair.getAmount());
                            chargebackAmount = chargebackAmount + amount;
                        } else if ("10004".equals(repair.getState())) {  //转单状态
                            //获取数量
                            int amount = Integer.parseInt(repair.getAmount());
                            transferOrderAmount = transferOrderAmount + amount;
                        } else if ("10006".equals(repair.getState())) {  //派单状态
                            int amount = Integer.parseInt(repair.getAmount());
                            dispatchAmount = dispatchAmount + amount;
                        } else if ("10008".equals(repair.getState())) {  //已回访状态
                            int amount = Integer.parseInt(repair.getAmount());
    ...
    public ResponseEntity<String> queryPayFeeDeposit(ReportDeposit reportDeposit) {
        ...
        for (ReportDeposit reportDeposit1 : reportDepositAmounts) {
            ...
            if (StringUtil.isEmpty(reportDeposit1.getDetailState())) { //获取未缴费总金额
                unpaidfeeAmounts = bd;
            } else if ("1000".equals(reportDeposit1.getDetailState())) { //获取退费中总金额
                refundInProgressAmounts = bd;
            } else if ("1100".equals(reportDeposit1.getDetailState())) { //获取已退费总金额
                refundedAmounts = bd;
            } else if ("1200".equals(reportDeposit1.getDetailState())) { //获取退费失败总金额
                refundFailedAmounts = bd;
            } else if ("1400".equals(reportDeposit1.getDetailState())) { //获取已缴费总金额
                paidfeeAmounts = bd;
            }
        }
