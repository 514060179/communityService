package com.newland.property.core.cache;
 ...
 * 自定义redis 缓存器
 */
public class NewlandPropertyRedisCacheWriter implements RedisCacheWriter {
     ...
     */
    NewlandPropertyRedisCacheWriter(RedisConnectionFactory connectionFactory) {
     ...
     *                          to disable locking.
     */
    NewlandPropertyRedisCacheWriter(RedisConnectionFactory connectionFactory, Duration sleepTime) {
    ...
    public void put(String name, byte[] key, byte[] value, @Nullable Duration ttl) {
        ...
        execute(name, connection -> {
            ...
            int index = name.lastIndexOf(NewlandPropertyRedisConfig.REDIS_EXPIRE_TIME_KEY);
            if (index > 0) {
                //取出对应的时间 1200 index + 1是还有一个=号
                String expireString = name.substring(index + 1 + NewlandPropertyRedisConfig.REDIS_EXPIRE_TIME_KEY.length());
