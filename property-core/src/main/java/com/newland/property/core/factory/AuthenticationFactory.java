...
import com.newland.property.dto.reportData.ReportDataHeaderDto;
import com.newland.property.dto.system.AppRoute;
import com.newland.property.utils.cache.AppRouteCache;
import com.newland.property.utils.cache.CommonCache;
import com.newland.property.utils.cache.JWTCache;
...
import com.newland.property.utils.exception.NoAuthorityException;
import com.newland.property.utils.util.Base64Convert;
...
public class AuthenticationFactory {
    ...
    public final static String PASSWD_SALT = "newland@property";
...
//            }
    ...
    public static void deleteToken(String token) throws Exception {
        ...
        Algorithm algorithm = Algorithm.HMAC256(jwtSecret);
        JWTVerifier verifier = JWT.require(algorithm).withIssuer("newland").build();
     ...
     * @throws Exception
     */
    public static Map<String, String> verifyToken(String token, String appId) throws Exception {
        String jwtSecret = MappingCache.getValue(MappingConstant.KEY_JWT_SECRET);
        if (StringUtil.isNullOrNone(jwtSecret)) {
            jwtSecret = CommonConstant.DEFAULT_JWT_SECRET;
        }
        ...
        // 判断是否为最新token
        String cacheKey = CommonConstant.USER_LATEST_JWT_TOKEN + userId;
        String latestToken = CommonCache.getValue(cacheKey);
        String oneDeviceLoginWhiteList = MappingCache.getValue(MappingConstant.ONE_DEVICE_LOGIN_WHITE_LIST);
        if(StringUtil.isNullOrNone(oneDeviceLoginWhiteList)) {
            oneDeviceLoginWhiteList = CommonConstant.DEFAULT_ONE_DEVICE_LOGIN_WHITE_LIST;
        }
        if (!StringUtil.isNullOrNone(latestToken) && !oneDeviceLoginWhiteList.contains(appId) && !token.equals(latestToken) ) {
            String message = MappingCache.getValue(MappingConstant.DOMAIN_COMMON_LOGIN_OTHER_DEVICE);
            if (StringUtil.isNullOrNone(message)) {
                message = CommonConstant.DEFAULT_LOGIN_OTHER_DEVICE;
            }
        ...
        if (!StringUtil.isEmpty(latestToken)) {
            CommonCache.resetExpireTime(cacheKey, Integer.parseInt(expireTime));
        }
